from time import *                         #importing time function
...
def mistake(paratest,usertest):            #created a mistake function 
    ...
    for i in range(len(paratest)):  
        try:
            if paratest[i] != usertest[i]: #checking the error
             error = error + 1
        except:
             error = error + 1
    return error        
...
def speed_time(time_s,time_e,userinput):  #created speed function 
    ...
    time_R = round(time_delay,2)          #rounding the time up to 2 digits
...
test1 = r.choice(test)                #generating random string from test 
print(" **** Typing Speed Test ****") 
print(test1)                          #printing the random test string
print()                               #used for spacing
print()
time_1 = time()
testinput = input(" Enter : ")       #taking userinput
print()
time_2 = time()
